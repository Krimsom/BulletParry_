...
public class PlayerController : MonoBehaviour
{
    ...
    [SerializeField] private float initialGravity = 5;
    ...
    void Start()
    {
        ...
        initialGravity = theRB.gravityScale;
    ...
    IEnumerator Dash()
    {
        isDashing = true;
        float dashTimer = 0;
        var previousVelocity = new Vector2(theRB.velocity.x, theRB.velocity.y);
        if (isLeft)
        {
           theRB.velocity -= new Vector2(dashDistance, 0);
        }
        else
        {
            theRB.velocity += new Vector2(dashDistance, 0);
        ...
        theRB.velocity = previousVelocity;
        isDashing = false;
    }
    ...
    // Método que se llama cuando se detecta una colisión con el collider de parry
    public void OnParry(Collider2D otherCollider)
    {
        // Si el collider de la bala está en capa "Bullets"
        if (otherCollider.gameObject.layer == LayerMask.NameToLayer("Bullets"))
        {
            // Detener el movimiento de la bala
            Rigidbody2D bulletRigidbody = otherCollider.GetComponent<Rigidbody2D>();
